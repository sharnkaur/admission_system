...
class MainWindow:
    def __init__(self):
        ...
        self.can=Canvas(self.fr,width=200,height=200)
        self.can.place(x=200,y=90)
        self.img=PhotoImage(file='./images/image1p.gif')
        self.can.create_image(0,0,image=self.img,anchor=NW)
          
        self.grid=Frame(self.fr,height=768,width=600,bg="#063852")
        self.table=Treeview(self.grid,column=('#0','#1','#2','#3','#4','#5'))
        style=ttk.Style()
        style.theme_use("alt")
        ...
        self.table.heading('#6',text="Full Details")
        ...
        print(y)
        for i in y:
            self.table.insert('','end',text=i[0],values=(i[1],i[2],i[3],i[4],i[5],i[6]))

        self.table.place(x=0,y=0)
        self.fr.place(x=0,y=0)
        self.grid.place(x=60,y=320)
